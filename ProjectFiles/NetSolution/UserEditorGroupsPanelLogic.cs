...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, Tata_EMS_Solution.ObjectTypes.GroupCheckbox1);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, Tata_EMS_Solution.ObjectTypes.GroupLabel1);
